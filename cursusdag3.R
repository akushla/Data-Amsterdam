...
dataAms $logbeveenouderhh <- log(dataAms$beveenouderhh_p)
hist(data)Ams$logbeveenouderhh)
...
bevmarok_p,bevsur_p,bevantil_p, bevovnw_p, bevman_p, bevvrouw_p, 
bevpotbbv15_64_p, bevopllaag_p, bevoplmid_p, bevoplhoog_p, wcorhuur_p,
wparthuur_p, wkoop_p)))
...
venn.diagram(
  ...
  category.names = c("1-oudergez", "Surinaamse afkomst","laagopgeleid"),
...
ggplot(dataStad,aes (x=as.factor, y=jaar))+
...
fit=lm(log(beveenouderhh_p+1)~ bevturk_p+bevmarok_p+bevsur_p+bevantil_p+
      bevovnw_p+bevman_p+bevvrouw_p+
      bevpotbbv15_64_p+bevopllaag_p+bevoplmid_p+bevoplhoog_p+wcorhuur_p+
      wparthuur_p+wkoop_p,
      data=dataAms)
summary(fit)
...
+ 49* fit$transterug [(3)] + 
  1*fit$transterug [(4)]+ 20*fit$transterug [(5)]+ 1*fit$transterug [(6)]+
  1*fit$transterug [(7)]+ 1*fit$transterug [(8)]+1*fit$transterug [(9)]+
  1*fit$transterug [(10)]+ 1*fit$transterug [(11)]+ 1*fit$transterug [(12)]+ 
  1*fit$transterug [(13)]+ 1*fit$transterug [(14)]
...
ggplot(dataAms,aes(y=beveenouderhh_p,x=bevopllaag_p))+
  geom_point(aes(color=gebiedcode15))+
  geom_smooth(method=lm)
...
dataMinZO <- filter(dataAms,gebiedcode15<100)
...
fitMinZO=lm(trans ~
          bevturk_p+bevmarok_p+bevsur_p+bevantil_p+
         bevovnw_p+bevman_p+bevvrouw_p+
         bevpotbbv15_64_p+wcorhuur_p+
         wparthuur_p+wkoop_p,
       data=dataMinZO)
summary(fitMinZO)
